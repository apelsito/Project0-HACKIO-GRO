...
select "Title" from "Album" where "AlbumId" = (Select);

-- Ejercicio 3:  Calcula la duración promedio, desviación estándar y varianza por compositor.
select "Composer", avg("Milliseconds") as "Promedio", stddev("Milliseconds") as "Desviación_estándar", variance("Milliseconds") as "Varianza" from "Track" group by "Composer"  ;
...
select "Country", count(*) as "Clientes" from "Customer" group by "Country" order by "Clientes" desc;

-- Ejercicio 9: Obtén el total de importes de facturas para cada país y muestra solo aquellos con un total superior a 10,000.
select "BillingCountry" , sum("Total") as "Importe" from "Invoice" group by "BillingCountry" having sum("Total") > 10000; --no hay ninguno, voy a asumir que es así xD

-- Ejercicio 10: Encuentra los países donde el número total de clientes es mayor a 5. Muestra el país y el número de clientes.
select "Country", count(*) as "Clientes" from "Customer" group by "Country" having count(*) > 5 order by "Clientes" desc ;
...
select "MediaTypeId", sum("Milliseconds") as "Duracion" from "Track" group by "MediaTypeId" having sum("Milliseconds") > 250000;
...
select "Country",count(*) as "Clientes Asistidos" from "Customer" where "SupportRepId" < 5 group by  "Country";

-- Ejercicio 13: Muestra los códigos postales (BillingPostalCode) de los clientes que han hecho más de 5 compras.
select "BillingPostalCode" from "Invoice" group by "BillingPostalCode" having count("InvoiceId") > 5;
...
select "MediaTypeId",count(*) as "Pistas" from "Track" where "UnitPrice" > 0.99 and "Milliseconds" > 200000 group by "MediaTypeId";
...
select "Country", "State", count(*) as "Numero_Clientes" from "Customer" where "SupportRepId" > 3 and "Company" notnull group by"Country","State";
